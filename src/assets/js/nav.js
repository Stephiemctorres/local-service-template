(() => {
	...
	// Add a shadow under the header once the page is scrolled, so it stays
	// flat/clean at the very top and gently separates from content below.
	const header = document.getElementById("cs-navigation");
	if (header) {
		const updateScrollShadow = () => {
			header.classList.toggle("cs-scrolled", window.scrollY > 8);
		};
