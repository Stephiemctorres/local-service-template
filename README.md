...
8.  Replace the placeholder favicon set in `./src/assets/favicons/` (all sizes, plus `favicon.svg`) with the client's real logo mark, and update `name`/`short_name` in `./src/assets/favicons/site.webmanifest`. This file is plain JSON, not a template — it's the one identity field in the repo that has to be edited by hand rather than pulled from `client.js`.
9.  Replace `./src/assets/images/og-share-placeholder.png` (1200×630) with a real social-share image before launch — this is what shows up when the site's links are shared on social media or messaging apps.
10.  Modify the website files (use `./src`, NOT `./public`) as needed. Use the template file in `content/pages/_template.txt` to get started, or modify the existing pages.
11.  Deploy using your preferred hosting provider.
